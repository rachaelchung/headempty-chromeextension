/**
 * Timer UI only. Talks to the bottom lane via parent.postMessage (content script → lane iframe).
 */
...
(function () {
  ...
  function toLane(type, value) {
    const d = { source: 'henn-timer', target: 'lane', type };
  ...
  function applyFocusGuard() {
    ...
    if (!focus) {
      toLane('setForbidden', false);
      setForbiddenBar(false);
      if (pausedForBadUrl) {
    ...
    if (badUrl) {
      toLane('setForbidden', true);
      setForbiddenBar(true);
